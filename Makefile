...
run: build ## Run the microservice in a container
	docker run -p 6004:6004 -v $(shell pwd)/.env:/.env -d ghcr.io/multimoml/stats:latest

build: tidy ## Build the Docker image
	docker build -t ghcr.io/multimoml/stats:latest .

push: build ## Manually push the Docker image
	docker push ghcr.io/multimoml/stats:latest
